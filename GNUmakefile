...
# ./internal/acceptance/$${f}/acc-testcases
...
accframework: vendor
	...
	for f in $(ACC_TEST_SERVICES); do \
		...
		# concatonate acc-testcases yaml files into on large temporary config file ; \
		for dir in $(TESTCASE_DIRS); do \
			touch ./internal/acceptance/$${f}/acc-testcases/$${dir}_temp_config.yaml ; \
			cat ./internal/acceptance/$${f}/acc-testcases/$${dir}/* >> ./internal/acceptance/$${f}/acc-testcases/$${dir}_temp_config.yaml ; \
		done ; \
